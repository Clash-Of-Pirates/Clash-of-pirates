...
impl UltraHonkVerifierContract {
    ...
    pub fn verify_proof(env: Env, vk_json: Bytes, proof_blob: Bytes) -> Result<BytesN<32>, Error> {
       ...
       // Convert proof_blob back to Bytes for verify call
        // Split inputs and proof, then convert back to Bytes
        let (pub_inputs_bytes_vec, proof_bytes_vec) = Self::split_inputs_and_proof_bytes(&proof_vec);
        
        // Flatten public inputs into a single byte vector
        let mut pub_inputs_flat = StdVec::new();
        for pi in pub_inputs_bytes_vec {
            pub_inputs_flat.extend_from_slice(&pi);
        }
        
        // Create NEW Bytes instances using the SAME env reference
        let proof_bytes = Bytes::from_slice(&env, &proof_bytes_vec);
        let pub_inputs_bytes = Bytes::from_slice(&env, &pub_inputs_flat);
