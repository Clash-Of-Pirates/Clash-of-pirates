import { Client as ClashClient, type Game } from './bindings';
...
/**
 * Service for interacting with the Clash game contract
 */
export class ClashService {
  private baseClient: ClashClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new ClashClient({
  ...
  private createSigningClient(
    ...
    return new ClashClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new ClashClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
...
// Example: const clashService = new ClashService(contractId);
