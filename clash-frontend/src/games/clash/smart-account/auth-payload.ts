/**
 * Auth payload helpers aligned with stellar-accounts 0.7 / OpenZeppelin smart accounts.
 * Ported from kalepail/smart-account-kit (auth-payload.ts) for use with smart-account-kit@0.2.x
 * which still signs the legacy hash-only challenge.
 */
import { Address, hash, xdr } from '@stellar/stellar-sdk';
import type { AuthPayload, Signer, WebAuthnSigData } from 'smart-account-kit-bindings';
import { signersEqual } from 'smart-account-kit';
...
function signerToScVal(signer: Signer): xdr.ScVal {
...
function parseSignerScVal(value: xdr.ScVal): Signer {
  if (value.switch().name !== 'scvVec') {
